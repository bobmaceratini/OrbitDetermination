...
def calculate_vectors(RA1,Dec1,t1,RA2,Dec2,t2,RA3,Dec3,t3,R1,R2,R3,R1dot,R2dot,R3dot):
    ...
    eps = (23.5*pi)/180
    rmatrix = np.array([[r2[0]],[r2[1]],[r2[2]]])
    rdotmatrix = np.array([[rdot_new[0]],[rdot_new[1]],[rdot_new[2]]])
    transformmatrix = np.array([[1,0,0],[0,cos(eps),sin(eps)],[0,-sin(eps),cos(eps)]])
...
def determine_orbital_elements(r,rdot):
    ...
    l = cross(r,rdot)
    print("l= ",l)
    irads = atan(sqrt(l[0]**2 + l[1]**2)/(l[2]+10**-22))
    i = irads*180/pi
    ...
    o1 = asin(l[0]/(mag(l)*sin(irads)+eps2))
    o2 = acos(-l[1]/(mag(l)*sin(irads)+eps2))
    print("o1= ", o1)
    print("o2= ", o2)
...
fileString = input("Please enter the name of the file: ")
